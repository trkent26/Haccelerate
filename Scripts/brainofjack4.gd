...
func _ready():
	...
	#connect("body_enter", self, "_on_Jack_body_enter")
	isAlive = true
...
func _fixed_process(delta):
	...
	#if get_linear_velocity().x < 0 and get_linear_velocity().y == 0:
...
#			set_axis_velocity(Vector2(0, -jump_height))
	...
	if Input.is_action_pressed("ui_menu"):
		get_tree().set_pause(true)
		get_node("/root/LevelFour").add_child(node)
	
	if get_tree().is_paused() == false:
		if get_node("/root/LevelFour").is_a_parent_of(node):
			get_node("/root/LevelFour").remove_child(node)
...
#		laserPos.x = jackPos.x
#		laserPos.y = jackPos.y
...
#	laserCount += 1
#	print("fire")
#	var laser_instance = bullet.instance()
#	laser_instance.set_name("laser" + str(laserCount))
#	get_node("/root/LevelOne").add_child(laser_instance)
#	var laserPos = get_node("laser" + str(laserCount)).get_pos()
#	var jackPos = get_node("Jack").get_pos()
#	laserPos.x = jackPos.x
#	laserPos.y = jackPos.y
