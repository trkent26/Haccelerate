...
func _process(delta):
	...
	if Input.is_action_pressed("ui_menu"):
		get_tree().set_pause(true)
		get_node("/root/LevelThree").add_child(node)
	
	if get_tree().is_paused() == false:
		if get_node("/root/LevelThree").is_a_parent_of(node):
			get_node("/root/LevelThree").remove_child(node)
...
func _on_Jack_body_enter( body ):
	if body.get_name() == "nextLevel":
		...
		print(endtime)
		Globals.set("LevelThreeTime", endtime)
		get_tree().change_scene("res://levelfour.tscn")
