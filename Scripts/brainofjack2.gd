...
#			set_axis_velocity(Vector2(0, -jump_height))
	...
	if Input.is_action_pressed("ui_menu"):
		get_tree().set_pause(true)
		get_node("/root/LevelTwo").add_child(node)
	
	if get_tree().is_paused() == false:
		if get_node("/root/LevelTwo").is_a_parent_of(node):
			get_node("/root/LevelTwo").remove_child(node)
...
func _on_Jack_body_enter( body ):
	if body.get_name() == "nextLevel":
		...
		print(endtime)
		Globals.set("LevelTwoTime", endtime)
